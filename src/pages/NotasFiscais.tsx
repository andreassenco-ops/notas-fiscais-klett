...
function buildQuery(dateFrom: string, dateTo: string): string {
  ...
  STUFF((
    SELECT ', ' + ISNULL(NULLIF(LTRIM(RTRIM(E.DESCRICAO)), ''), SE.EXAME)
    FROM SOLICITACAO_EXAMES_GUIA SEG2
    INNER JOIN SOLICITACAO_EXAMES SE ON SE.ID = SEG2.SOLICITACAO_EXAMES_ID
    LEFT JOIN EXAMES E ON E.SIGLA = SE.EXAME
    WHERE SEG2.SOLICITACAO_GUIA_ID = SOLICITACAO_GUIA.ID
    ORDER BY SEG2.ORDEM, SEG2.ID
    FOR XML PATH(''), TYPE
